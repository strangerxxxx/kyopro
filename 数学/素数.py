def faster_eratosthenes(n: int) -> list:
    """
    ...
    for k in range(max_sqrt_k + 1):
        for m in range(8):
            ...
            m_before = m
            if table[k] & shift[m]:
                i = k * (30 * k + 2 * remains[m]) + div30[m][m]
                j = mod30[m][m]
                while i < max_k or (i == max_k and j <= max_m):
                    table[i] &= msk8[j]
                    if m_before == 7:
                        i += 2 * k + remains[m] + div30[m][0] - div30[m][7]
                        j = mod30[m][0]
                        # k_before += 1
                        m_before = 0
                    else:
                        i += k * (remains[m_before + 1] - remains[m_before]) + \
                            div30[m][m_before + 1] - div30[m][m_before]
                        j = mod30[m][m_before + 1]
                        m_before += 1
    for i in range(max_k + 1):
        while table[i]:
            j = inv_shift[table[i] & (-table[i])]
            res.append(i * 30 + remains[j])
            table[i] &= table[i] - 1
    return res
