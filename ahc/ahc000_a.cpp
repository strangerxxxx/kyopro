...
#pragma endregion
constexpr double timelimit = 1.99;
// Optunaを使うとき用
...
int main() {
    ...
    // iota(ALL(nowans), 0);
    VI bestans = nowans;
    auto calcscore = [&](VI ans) -> long long {
        // スコア関数
        return 0LL;
    };
    constexpr double starttemp = 1e4, endtemp = 1e2;
    constexpr double difftemp = starttemp - endtemp;
    // Optunaを使うとき用
    // double starttemp = 1e4, endtemp = 1e2;
    // if (argc == 3) {
    //     starttemp = atof(argv[1]), endtemp = atof(argv[2]);
    // }
    // double difftemp = starttemp - endtemp;
    double time = TIME();
    auto calctemp = [&time, endtemp, difftemp]() -> double {
        return endtemp + difftemp * (1 - time / timelimit);
    };
    double temp = calctemp();
    // 焼きなましを採用するか判定
    auto trabs = [&](int newscore, int prescore) -> bool {
        temp = starttemp - (starttemp - endtemp) * TIME() / timelimit;
        // 最小化ならprescore - newscoreにする
        return exp((newscore - prescore) / temp) >
               double(XORShiftRandom()) / UINF32;
    };
    // 閾値を前計算するときの関数
    // auto diff_threshold = [&temp]() -> int {
    //     // return 0
    //     return temp * log(double(XORShiftRandom()) / UINF32);
    // };
    long long nowscore = calcscore(nowans), bestscore = nowscore, score;
    while (time < timelimit) {
        REP(_, 100) {
            score = calcscore(nowans);
            if (trabs(score, nowscore)) {
                nowscore = score;
                // 最小化なら符号を入れ替える
